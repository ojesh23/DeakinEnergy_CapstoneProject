...
################## Random Forest Encoder and Model ##################
RF_MODEL_PATH = 'Models/T3/RF/random_forest.pickle'
TFIDF_ENCODER_PATH = 'Models/T3/RF/TFIDF_1000.pickle'

# load model and encoder
rf_model = pickle.load(open(RF_MODEL_PATH, 'rb'))
tfidf_1000 = pickle.load(open(TFIDF_ENCODER_PATH, 'rb'))

################## Random Forest Prediction ##################
def random_forest(text):
    """
    This function takes a whole text and performs preprocessing of the text,
    use tfidf with max_features=1000 then makes prediction
    returns: prediction category and probability
    """
...
def predict_one_by_one():
    ...
    causes = str(request.form.get('causes'))
    consequences = str(request.form.get('consequences'))

    # concat all user responses
    full_input = incident + ' ' + causes + ' ' + consequences
    ...
    if (classifier_name == "rf"):
        random_forest = RandomForest(full_input)
        rf_pred, rf_prob = random_forest.rf_prediction()
    ...
    return (render_template('index3.html', scroll='table', incident=incident, causes=causes, consequences=consequences, xgb_pred=xgb_pred, xgb_prob=xgb_prob,
                            lr_pred=lr_pred, lr_prob=lr_prob, svm_pred=svm_pred, svm_prob=svm_prob, rf_pred=rf_pred,
                            rf_prob=rf_prob, rnn_pred=rnn_pred, rnn_prob=rnn_prob, knn_pred=knn_pred, knn_prob=knn_prob,
...
def predict():  # predicts all available classifiers at the same time
    ...
    svm_pred, svm_prob = svm_predictor(incident)
    # Random Forest
    rf_pred, rf_prob = random_forest(full_input)
    # Decision Tree
    dt_pred, dt_prob = dt_predictor(incident)
    # RNN
    rnn_pred, rnn_prob = rnn_predictor(incident)
    ...
    return (render_template('index3.html', scroll='table', incident=incident, xgb_pred=xgb_pred, xgb_prob=xgb_prob,
                            lr_pred=lr_pred, lr_prob=lr_prob, svm_pred=svm_pred, svm_prob=svm_prob, rf_pred=rf_pred,
                            rf_prob=rf_prob, rnn_pred=rnn_pred, rnn_prob=rnn_prob, knn_pred=knn_pred, knn_prob=knn_prob,
                            dt_pred=dt_pred, dt_prob=dt_prob))
...
class XGBoost(Resource):
    ...
    def get(self):
        ...
        custom = ["reported", "found", "caused", "incident", "crew", "injuries", "location", "arrival", "grassfire",
                  "fire", "ground", "pole"]
        for word in custom:
            stopwords.append(word)

        # Splitting of data in test and train
        x_train, x_test, y_train, y_test = train_test_split(df['EventDescription'], df['Category'], test_size=0.3)
...
class KNN(Resource):
    ...
    def post(self):
        ...
        text_clf = Pipeline([('vect', CountVectorizer()), ('tfidf', TfidfTransformer()), ('clf', knn), ])
...
class SVM(Resource):
    ...
    def post(self):
        df = pd.read_csv(
            r'C:\Users\Admin\Documents\d2i---deakin-energy_2020t2\Datasets\ESV Data\cleaned_incidents1.csv')
        ...
        wv = gensim.models.KeyedVectors.load_word2vec_format(
            r"C:\Users\Admin\Documents\d2i---deakin-energy_2020t2\Data Analysis\Deliverable 3\GoogleNews-vectors-negative300.bin.gz",
            binary=True)
...
class NeuralNetwork(Resource):
    ...
    def post(self):
        df_esv = pd.read_csv(
            'esv_EventDescription_tokenized_encoded2.csv')  # one hot encoded data , category is first col
                    ...
                    for lr in learning_rates:
                        for lf in loss_functions:
                            for bs in batch_sizes:
                                ...
                                history = nn.fit(X_train, y_train_enc, epochs=epoch, batch_size=bs,
                                ...
                                results.append(
                                    {'Epoch': epoch, 'BatchSize': bs, 'Optimizer': optimizer, 'Dropout': dropout,
                                     'LearningRate': lr,  # todo use learning rate
                                     'LossFunction': lf, 'Accuracy': result[1], 'History': history})
...
class DecisionTree(Resource):
    ...
    def post(self):
        col_names = ['ActionTaken', 'Address', 'AssetLabel', 'CauseCommunity', 'CauseEnvironment', 'CausePre',
                     'CauseTechnical', 'CauseWorkP', 'ContactType', 'CorrectProtection', 'EventDescription',
                     'FailedAssets', 'FailedExplosion', 'FailedOilFilled', 'FailedOtherAssets',
                     'FailedOtherAssetsOther', 'FeederNumber', 'IncidentCause', 'IncidentConsequence',
                     'IncidentDatetime', 'IncidentFireFFactorReportable', 'IncidentFireSeverity', 'IncidentID',
                     'IncidentLocationType', 'IncidentLocationTypeOther', 'IncidentNumber', 'IncidentType', 'Lat',
                     'Long', 'MadeSafe', 'NetworkType', 'Status', 'SubmissionID', 'SubmittedDateTimeString', 'Voltage',
                     'WeatherStation', 'Postcode', 'Locality', 'Category']
        # load dataset
        ...
        def clean_text(text):
            text = text.lower()  # lowercase text
            text = REPLACE_BY_SPACE_RE.sub(' ',
                                           text)  # replace REPLACE_BY_SPACE_RE symbols by space in text.  substitute the
            # matched string in
            # REPLACE_BY_SPACE_RE
            # with
            # space.
...
print(f'Time taken to load Web App: {difference} seconds')
